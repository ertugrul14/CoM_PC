...
def _predict_street_mean(model, cube_norm, node_idx, feat_names, norm_stats,
    ...
    with torch.no_grad():
        ...
        for s in starts:
            ...
            p = model(X)[0][0, node_idx, 0].item()         # [0]=ped head (dual-head); scalar (normalised)
            # D-012: expm1 de-norm for log-normalised ped_flow (per-sample, before mean)
            preds.append(float(denormalise_feature(p, "ped_flow", norm_stats)))
...
def run() -> dict[str, Path]:
    ...
    # D-012: ped_flow log1p de-norm is nonlinear → de-norm each cell to raw,
    # THEN average over time (mean-then-expm1 would be biased). occ stays linear.
    ped_mean_raw = denormalise_feature(ped_val_norm, "ped_flow", norm_stats).mean(axis=1)  # [N]
    ...
    with torch.no_grad():
        for s in starts:
            ...
            baseline_preds.append(model(X_b)[0][0, :, 0].cpu().numpy())  # [0]=ped head; [N]
            ...
            cf_preds.append(model(X_cf)[0][0, :, 0].cpu().numpy())       # [0]=ped head; [N]

    # D-012: de-norm each sample to raw ped (expm1) BEFORE averaging across windows
    baseline_mean = denormalise_feature(np.stack(baseline_preds), "ped_flow", norm_stats).mean(axis=0).clip(0)  # [N]
    cf_mean       = denormalise_feature(np.stack(cf_preds),       "ped_flow", norm_stats).mean(axis=0).clip(0)  # [N]
