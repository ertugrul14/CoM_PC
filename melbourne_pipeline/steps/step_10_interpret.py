...
def _baseline_mae(model, cube_norm, T_train, T, target_fi, norm_stats,
                  feat_names, device, n_eval=N_EVAL):
    ...
    with torch.no_grad():
        for _ in range(0, n_eval, BATCH_SIZE):
            X, y_n = _sample_windows(cube_norm, T_train, T, BATCH_SIZE,
                                     WINDOW, target_fi, device)
            p = model(X).cpu().numpy() * std + mu
            all_pred.append(np.clip(p, 0, None))
            all_y.append(y_n.cpu().numpy() * std + mu)
...
def run() -> dict[str, Path]:
    ...
    baseline = _baseline_mae(model, cube_norm, T_train, T,
                              target_fi, norm_stats, feat_names, device)
    ...
    for fi, name in enumerate(feat_names):
        ...
        for _ in range(N_REPEATS):
            mae_p = _perturbed_mae(model, cube_norm, T_train, T, target_fi,
                                   fi, norm_stats, feat_names, device)
