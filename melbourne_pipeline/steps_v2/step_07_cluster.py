...
Hard sensor gate (V2):
  A flexibility window is a conjunction of two signals — high ped demand AND low
  parking occupancy. The kerb-slack (parking) half is what a reallocation actually
  consumes, so a recommendation is only emitted where parking is REALLY observed.
  Each street is assigned an evidence tier; only tiers backed by a real parking
  sensor are flagged as reallocation candidates. Imputed streets keep their windows
  for visualisation but are labelled indicative (not actionable).

Evidence tiers:
  A_confirmed           — ped sensor AND park sensor   (both halves observed)   -> candidate
  B_park_provisional    — park sensor only             (slack real, demand modelled) -> candidate
  C_ped_only_indicative — ped sensor only              (demand real, slack unobserved) -> NOT
  D_indicative          — neither sensor               (both halves modelled)   -> NOT

Outputs (data/processed_v2/):
  clustered.parquet          — + evidence_tier, ped_sensor, park_sensor, reallocation_candidate
  cluster_centroids.csv      — mean profile per cluster in original feature space
  cluster_report.json        — k, BIC, ARI, silhouette, cluster sizes, archetype map, tier counts
"""
import json
import logging
...
# V2 outputs are isolated from the frozen pipeline (do not clobber clustered.parquet)
...
STABILITY_ITERS      = 20
SEED                 = 42
PED_SENSOR_CONF      = 1.0    # ped_confidence == 1.0 marks a real pedestrian sensor (D-017)
NIGHT_BLOCK          = "night" # overnight block excluded from flexibility windows (D-021)
...
def _sensor_membership(
    streets: list[str],
    ped_confidence: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    """
    Determine, per street, whether it carries a REAL pedestrian sensor and/or a
    REAL parking sensor.

    Inputs:
      streets        — ordered street_id list (matches the clustering row order)
      ped_confidence — per-street ped_confidence column (1.0 == real ped sensor)
    Outputs:
      (ped_sensor, park_sensor) — two bool arrays aligned to `streets`.

    Assumptions:
      - ped sensor  iff ped_confidence == 1.0 (D-017 convention).
      - park sensor iff street_id appears in parking_occupancy.parquet, whose
        street_id set is exactly the real parking-sensored streets (143 inside cube).
    """
    ped_sensor = np.isclose(np.asarray(ped_confidence, dtype=float), PED_SENSOR_CONF)

    park_df = pd.read_parquet(
        PROCESSED_DIR / "parking_occupancy.parquet", columns=["street_id"]
    )
    park_ids = set(park_df["street_id"].astype(str).unique())
    park_sensor = np.array([s in park_ids for s in streets], dtype=bool)

    return ped_sensor, park_sensor


def _evidence_tiers(
    ped_sensor: np.ndarray,
    park_sensor: np.ndarray,
) -> tuple[list[str], np.ndarray]:
    """
    Assign each street an evidence tier and a hard reallocation-candidate flag.

    The gate keys on the PARKING sensor: a reallocation consumes kerb slack, so the
    parking half of the flexibility test must be observed, not imputed. The ped
    sensor only distinguishes a confirmed (A) from a provisional (B) candidate.

    Returns (tier_labels, reallocation_candidate) aligned to the input arrays.
    """
    tiers: list[str] = []
    for has_ped, has_park in zip(ped_sensor, park_sensor):
        if has_park and has_ped:
            tiers.append("A_confirmed")
        elif has_park:
            tiers.append("B_park_provisional")
        elif has_ped:
            tiers.append("C_ped_only_indicative")
        else:
            tiers.append("D_indicative")
    candidate = park_sensor.copy()  # hard gate: candidate iff real parking slack
...
def _flexibility_windows(
    ...
    Window selection (D-021):
      C — overnight blocks are excluded from candidacy. Kerb is empty almost
          everywhere at night, so a "night" window is trivial and not an
          actionable reallocation (no daytime demand to serve).
      B — `best_window` is the qualifying block with the HIGHEST pedestrian
          demand (most people to serve), not the first one in column order.
          `flex_all` is returned ranked by the same demand, so flex_all[0]
          always equals best_window.
    """
    ...
    for i in range(len(labels)):
        ...
        scored: list[tuple[str, float]] = []   # (window_label, ped_demand)
        for pi, pcol in enumerate(park_cols):
            label = pcol.replace("park_occupancy_rate_", "")
            # C: skip trivial overnight blocks (reallocating empty 3am kerb is moot)
            if label.endswith(f"_{NIGHT_BLOCK}"):
                continue
            ...
            ped_demand = float(ped_arr[i, qi])
            if ped_demand > med[qi]:
                scored.append((label, ped_demand))
        # B: rank by pedestrian demand, strongest first
        scored.sort(key=lambda t: t[1], reverse=True)
        windows = [label for label, _ in scored]
...
def run() -> dict[str, Path]:
    ...
    # ── Hard sensor gate ──────────────────────────────────────────────────────
    # Tier each street by real sensor backing; only parking-sensored streets can
    # be reallocation candidates (the kerb-slack half must be observed, not imputed).
    ped_sensor, park_sensor = _sensor_membership(streets, profiles["ped_confidence"].values)
    evidence_tier, reallocation_candidate = _evidence_tiers(ped_sensor, park_sensor)

    # A candidate window only counts when the street is actually a candidate AND
    # the flexibility test fired — this is the actionable set RQ1 should report on.
    candidate_with_window = np.array(
        [bool(c) and bool(w) for c, w in zip(reallocation_candidate, flex_best)]
    )

    tier_counts = {t: int(evidence_tier.count(t)) for t in
                   ("A_confirmed", "B_park_provisional",
                    "C_ped_only_indicative", "D_indicative")}
    log.info(f"  Evidence tiers: {tier_counts}")
    log.info(f"  Reallocation candidates (real parking slack): {int(reallocation_candidate.sum())}")
    log.info(f"  Candidates WITH a flexibility window: {int(candidate_with_window.sum())}")
    ...
    result = pd.DataFrame({
        ...
        "ped_sensor":             ped_sensor.tolist(),
        "park_sensor":            park_sensor.tolist(),
        "evidence_tier":          evidence_tier,
        "reallocation_candidate": reallocation_candidate.tolist(),
    ...
    # ── Save (V2: isolated dir, frozen outputs untouched) ─────────────────────
    V2_DIR.mkdir(parents=True, exist_ok=True)
    clustered_path  = V2_DIR / "clustered.parquet"
    centroids_path  = V2_DIR / "cluster_centroids.csv"
    report_path     = V2_DIR / "cluster_report.json"
    ...
    report = {
        ...
        "n_reallocation_candidates": int(reallocation_candidate.sum()),
        "n_candidates_with_window":  int(candidate_with_window.sum()),
        "gate":                    "hard_park_sensor",
    }
    report_path.write_text(json.dumps(report, indent=2))

    log.info(f"  Streets with flexibility windows (all tiers): {report['n_with_flex_windows']}")
    log.info(f"  Actionable candidates with window (gated): {report['n_candidates_with_window']}")
    ...
    summ_cols = ["street_id", "cluster", "intervention_type", "cluster_confidence",
                 ...
                 "evidence_tier", "reallocation_candidate"]
    ...
    summ_path = V2_DIR / "cluster_summary.json"
